...
#'
pgee_run = function(y, X, n_subj, n_visits, covariance = "Independence", tol = 1e-4, max_iter = 10, phi_est = T){
  ...
  #beta_old = as.vector(glm(y~X[,-1], family = poisson(log),
  #               method = "brglmFit", type = "AS_mean", maxit = 10, epsilon = 1e-04, slowit=0.5)$coef)
  ...
  for (i in 1:max_iter) {
    ...
    switch(covariance,
           ...
           #   iter_alpha  = (sum_res / ((n_visits-1)))/(n_subj)
    ...
    #added on 15th March 2021
    #iter_U      = tmp5 %*% (y - iter_mu)
    
    # tmp_new1      = diag(sqrt(iter_mu[,1]), ncol=n_obs)
    # iter_V        = as.matrix(tmp_new1 %*% bdiag(rep(list(iter_R), n_subj)) %*% tmp_new1)
    # iter_H_term1  = sqrtm(iter_V)%*%X
    # iter_H_term2  = t(X) %*% iter_V %*% X
    # iter_H        = iter_H_term1 %*% spdinv(iter_H_term2) %*% t(iter_H_term1)
    # iter_U        = (bdiag(rep(list(solve(iter_R)), n_subj)) + 0.5*diag(iter_H, ncol=n_obs))%*% X
    
    # diag_H = rep(0, n_obs)
    # tmp_new1      = diag(sqrt(iter_mu[,1]), ncol=n_obs)
    # iter_V        = as.matrix(tmp_new1 %*% bdiag(rep(list(iter_R), n_subj)) %*% tmp_new1)
    # iter_H_term2  = t(X) %*% iter_V %*% X
    # iter_H_term2_inv = spdinv(iter_H_term2)
    # 
    # for (i in 1:n_subj) {
    #   idx_vec = ((i-1)*n_visits+1):(i*n_visits)
    #   #print(idx_vec)
    #   iter_H_term1  = sqrtm(iter_V[idx_vec, idx_vec])%*%X[idx_vec,]
    #   #print(iter_H_term1)
    #   # iter_H_term2  = t(X[idx_vec,]) %*% iter_V %*% X[idx_vec,]
    #   # print(iter_H_term2)
    #   iter_H        = iter_H_term1 %*% iter_H_term2_inv %*% t(iter_H_term1)
    #   #print(iter_H)
    #   diag_H[idx_vec]= diag(iter_H)
    #   #print(diag_H[idx_vec])
    #   #if(i%%100==0) print(i)
    # }
    # #print(diag_H)
    
    # iter_H_term1 = matrix(rep(0, n_obs*n_obs), ncol=n_obs, nrow=n_obs)
    # diag_H = rep(0, n_obs)
    # tmp_new1      = diag(sqrt(iter_mu[,1]), ncol=n_obs)
    # #iter_V        = as.matrix(tmp_new1 %*% bdiag(rep(list(iter_R), n_subj)) %*% tmp_new1)
    # for (i in 1:n_subj) {
    #   idx_vec = ((i-1)*n_visits+1):(i*n_visits)
    #   #print(idx_vec)
    #   iter_H_term1[idx_vec, idx_vec]  = solve(sqrtm(tmp_new1[idx_vec, idx_vec]%*%iter_R%*%tmp_new1[idx_vec, idx_vec]))
    # }
    # iter_H_term2  = iter_H_term1 %*%  tmp2
    # Qmat          = qr.Q(qr(iter_H_term2))
    # #print(dim(Qmat*Qmat))
    # diag_H        = as.vector(rowSums(Qmat*Qmat))
    # rm(tmp_new1, Qmat, iter_H_term1, iter_H_term2)
    # gc()
    # print("QR")
    # #print(dim(diag_H))
    # print(diag_H)
    # 
    # #added 21 Sep '21
    # H_full = tmp2 %*% solve(iter_I) %*% tmp3 %*% tmp4
    # print("full H")
    # print(dim(H_full))
    # print(diag(H_full))
    
    iter_I_inv = solve(iter_I) 
    H_IK = diag(iter_mu[,1], ncol=n_obs) %*% tmp4 %*% tmp2 %*% iter_I_inv %*% t(X)
    # print("IK H")
    # print(dim(H_IK))
    # print(diag(H_IK))
    
    #iter_R_inv = solve(iter_R)
    # iter_U        = t(X)%*% bdiag(rep(list(iter_R_inv), n_subj))%*%(y - iter_mu) + colSums(diag(diag_H/2) %*% X)
    iter_U        = tmp5 %*% (y - iter_mu) + colSums(diag(H_IK/2) %*% X)
